...
def tracking_entry_add():
    ...
    if d["entry_type"] not in ("bestand", "zaehler", "auffuellung", "entnahme"):
        abort(400, "entry_type muss 'bestand', 'zaehler', 'auffuellung' oder 'entnahme' sein")
...
def tracking_item_add():
    ...
    row = db_insert("""
        INSERT INTO tracking_items
            (category_id, slug, name, tracking_mode, base_unit, unit_size, presets, sort_order)
...
def tracking_item_update(item_id):
    d = request.get_json(force=True)
    allowed = ["name", "tracking_mode", "base_unit", "unit_size", "presets", "sort_order", "active", "category_id"]
    clauses, vals = [], []
    for f in allowed:
        if f in d:
            clauses.append(f"{f} = %s"); vals.append(d[f])
